...
if (abs(xerror)+abs(yerror))==0
    a="standby"; %Output next action is to stay still.
elseif herror<=90 || herror>=270 %Move forward
    ...
    if (((herror<=15) || (herror>=345))||((herror>=165)&&(herror<=195)))
        a="forward"; %Output next action should be to move forward
    elseif (((herror>15) && (herror<=90))||((herror>195)&&(herror<270)))
        a="leftforward"; %Output next action should be to move forward left
    elseif (((herror>=270) && (herror<345))||((herror>90)&&(herror<165)))
        a="rightforward"; %Output next action should be to move forward right
    end
else %Move Backward
    ...
    herror=mod(angled-hangle,360); %find error between current heading and desired heading
    if (((herror<=15) || (herror>=345))||((herror>=165)&&(herror<=195)))
        a="backward"; %Output next action should be to move backward
    elseif (((herror>15) && (herror<=90))||((herror>195)&&(herror<270)))
        a="leftbackward"; %Output next action should be to move backward left
    elseif (((herror>=270) && (herror<345))||((herror>90)&&(herror<165)))
        a="rightbackward"; %Output next action should be to move backward right
    end
end
